...
cp -R "$manager_copy_dir/example-skill" "$manager_copy_home/.agents/skills/example-skill"
...
retired_apply_output="$(expect_failure ruby "$repo_root/scripts/skills_sync.rb" --apply)"
assert_contains "$retired_apply_output" "invalid option: --apply"
...
assert_contains "$renamed_export_manager_output" "management=manual-review"
...
assert_contains "$missing_root_output" "consumer root is missing; upstream manager install is expected to create or repair it"
